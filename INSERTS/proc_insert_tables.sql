...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20003, 'Erro: O usuário especificado não existe.');
    WHEN DUP_VAL_ON_INDEX THEN
        RAISE_APPLICATION_ERROR(-20004, 'Erro: Já existe um endereço com este ID.');
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20005, 'Erro inesperado ao inserir endereço: ' || SQLERRM);
END;
...
CREATE OR REPLACE PROCEDURE SP_INSERIR_PONTUACAO_USUARIO (
